...
public class Step3 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Step currentStep = Steps.Step3.getStep(quotation.getQuotationItemsCurrentID());
		milestone.go(currentStep);

		// Uri
		String uriPrevious = request.getContextPath() + Steps.Step2.getStep(quotation.getQuotationItemsCurrentID()).getURI();
		String uriNext = request.getContextPath() + Steps.QuotationInfo.getStep(quotation.getQuotationItemsCurrentID()).getURI();
		
		request.setAttribute("uriprevious", uriPrevious);
		request.setAttribute("urinext", uriNext);
		
		request.getRequestDispatcher("/WEB-INF/product/step3.jsp").forward(request, response);
		return;
	}
