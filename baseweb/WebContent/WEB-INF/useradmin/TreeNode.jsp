...
<c:otherwise>                         								<%-- it is not root --%>
    ...
    <c:when test="${ node.children.size()>0 }">
          ...
          <div class="treeoftree-expand">
            ...
            <span style='${usercompany.id==node.id?"background-color: #ccc;":""}' 
                  onclick='post("${pageContext.request.contextPath}/UserCompanyManagement", { companyID : ${node.id} });'>${node.name}</span>
          </div>
	  	  <ul class="treeoftree">
        	<c:forEach var="node" items="${node.children}">
	      		<c:set var="node" value="${node}" scope="request" />
    ...
    <c:otherwise>
		<li><span style='${usercompany.id==node.id?"background-color: #ccc;":""}' 
		          onclick='post("${pageContext.request.contextPath}/UserCompanyManagement", { companyID : ${node.id} });'>${node.name}</span></li>
    </c:otherwise>
    </c:choose>
</c:otherwise>
</c:choose>
